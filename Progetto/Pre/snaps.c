...
typedef struct snap_elem{
    int time;
    struct snap_elem *prev, *next;
    snapshot_t snap;
}snapshot_list_t;
...
void TakeSnapshot(){
    ...
    if(snaps == NULL){
        snaps=malloc(sizeof(snapshot_list_t));
        snaps->next = NULL;
        snaps->prev = NULL;
        snaps->snap.size = t_len;
        snaps->snap.lines=(char**)malloc(t_len * sizeof(char*));
        for(int i = 0; i < t_len; i++){
            snaps->snap.lines[i] = text[i];
        }
    }
    else if(currentState >= snaps->time){
        snaps->next=malloc(sizeof(snapshot_list_t));
        snaps->next->prev = snaps;
        snaps = snaps->next;
        snaps->next = NULL;
        snaps->snap.size = t_len;
        snaps->snap.lines=(char**)malloc(t_len * sizeof(char*));
