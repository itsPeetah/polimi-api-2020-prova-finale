...
void OnRestoreActions(int steps);
...
void UpdateHistory()
{
    ...
    int lastState = stateCount - 1;

    history[lastState].undo = NULL;
    history[lastState].redo = NULL;
    history[lastState].restore = NULL;

    if(lastState%RESTORE_SAMPLE_FREQUENCY == 0)
    {
        history[lastState].restore = (edit_t*)realloc(history[lastState].restore, sizeof(edit_t));
        history[lastState].restore->code = RESTORE;
        history[lastState].restore->size = t_len;
        history[lastState].restore->setlen = t_len;
        history[lastState].restore->location = 0;
        history[lastState].restore->lines = NULL;
        history[lastState].restore->lines = (char**)realloc(history[lastState].restore->lines, t_len * sizeof(char*));
        // Copy lines
        for(int i = 0; i < t_len; i++){
            history[lastState].restore->lines[i] = text[i];
        }
    }
}
...
int CalculateTargetState(int steps)
{
    int target = currentState + steps;
    if(target < 0) target = 0;
    else if (target > stateCount - 1) target = stateCount - 1;
...
int FindClosestSamplePoint(int to)
{
    ...
    if(remainder > RESTORE_SAMPLE_FREQUENCY/2 && closest + RESTORE_SAMPLE_FREQUENCY < stateCount - 1) 
...
void OnRestoreActions(int steps)
{
    int targetState = CalculateTargetState(steps);
    int closestSamplePoint = FindClosestSamplePoint(targetState);

    if(abs(currentState-targetState) <= abs(closestSamplePoint-targetState))
    {
        //Steps to take
        steps = targetState-currentState;
    }
    // Current is further from the target then the closest sample point
    else
    {
        currentState = closestSamplePoint;
        int newLength = history[currentState].restore->setlen;
        SetTextLength(newLength);
        for(int i = 0; i<newLength; i++){
            text[i] = history[currentState].restore->lines[i];
        }

        // Steps to take
        steps = targetState-closestSamplePoint;
    }

    if(steps < 0) OnUndo(abs(steps));
    else if(steps > 0) OnRedo(steps);
}
