...
#define FULL_BACKUP_FREQUENCY 20
...
void SaveBackup(int state);
int FindClosestBackup(int target);
...
void OnChange(int from, int to){
    ...
    if(currentState% FULL_BACKUP_FREQUENCY != 0 || currentState == 0) // Don't do backup
    {
    ...
    {
        edit_t *undo = GetStateEdit(UNDO);
        SetupEdit(undo, RESTORE, 0, prevLen, 0);
        
        for(int i = 0; i <= max(to, prevLen); i++){
...
void OnDelete(int from, int to){
    if(from > t_len || to < 1) {
        ...
        edit_t *redo = GetStateEdit(REDO);
        if(currentState % FULL_BACKUP_FREQUENCY != 0 || currentState == 0)
            SetupEdit(redo, SKIP, 0, 0, 0);
        else
        {
            SetupEdit(redo, RESTORE, 0, t_len, 0);
            for(int i = 0; i < t_len; i++){
                AddLineToEdit(redo, &text[i]);
            }
        }
    ...
    {
        edit_t *undo = GetStateEdit(UNDO);
        SetupEdit(undo, RESTORE, 0, t_len, 0);
    
        for(int i = 0; i < t_len; i ++){

            AddLineToEdit(undo, &text[i]);

            if(i >= from - 1 && i + offset < t_len){
...
void TryRestoreState(){
    ...
    int closestBackup = FindClosestBackup(target);
    int startFrom = currentState;

    if(target < abs(target-startFrom)){
        startFrom = 0;
    }
    if(abs(target-closestBackup) < abs(target-startFrom)){
        startFrom = closestBackup;
    }
    if(rm_state > 0 && abs(target - rm_state) < abs(target-startFrom)){
        startFrom = rm_state;
    }
    ...
    //     SetTextLength(0);
    //     currentState = 0;
    // } else if (rm_state > 0 && startFrom == rm_state){
    ...
    // }
    // else if(startFrom == closestBackup){
    //     edit_t *e = history[closestBackup+1].undo;
    //     SetTextLength(e->setlen);
    //     for(int i = 0; i < t_len; i++)
    //         text[i] = e->lines[i];
    //     currentState = closestBackup;
    //     actions_to_restore = target-closestBackup;
    // }
...
int FindClosestBackup(int target)
{
    int lesser = target - (target%FULL_BACKUP_FREQUENCY);
    int greater = lesser + FULL_BACKUP_FREQUENCY;
    int closest = abs(target - lesser) <= abs(target-greater) ? lesser : greater;
    return closest < stateCount - 1 ? closest : lesser;
}