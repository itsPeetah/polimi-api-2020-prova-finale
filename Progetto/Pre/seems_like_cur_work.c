/*
    Branching in history causes undos to lose precision
*/
...
void AddLineToEdit(edit_t *e, char **lineContent, int index, int increaseSize);
...
void UpdateHistory()
{
    // This is the first state being generated or it is the last one of the timeline
...
void SetupEdit(edit_t *e, char code, int loc, int setl, int size)
{
    ...
    e->setlen = setl;
    e->size = size;
    e->lines = size > 0 ? (char**)realloc(e->lines, size * sizeof(char*)) : NULL;
}
...
void OnChange(int from, int to)
{
    ...
    for(int i = from; i <= to; i++){
        ...
        if(i <= prevLen)
        {
            AddLineToEdit(undo, &text[i-1], i-1, 1);
        ...
        AddLineToEdit(redo, &in_buf, i-1, 1);
    }
