...
void UpdateHistory()
{
    ...
    // Adjust count
    if(count == 0 || count == current + 1) count++;
    else 
    {
    ...
    // Adjust capacity
    if(count > capacity) capacity += STATE_BLOCK_SIZE;
    else if(count <= capacity - STATE_BLOCK_SIZE) capacity -= STATE_BLOCK_SIZE;

    // Reallocate 
    history = (state_t*)realloc(history, capacity * sizeof(state_t));
    ...
    history[count - 1].undo.code = UNINITIALIZED;
    history[count - 1].redo.code = UNINITIALIZED;
...
void FreeStateContent(int index){
    ...
    if(history[index].undo.code != UNINITIALIZED)
    {
        free(history[index].undo.lines);
        history[index].undo.code = UNINITIALIZED;
    }
    
    if(history[index].redo.code != UNINITIALIZED)
    {
        free(history[index].redo.lines);
        history[index].redo.code = UNINITIALIZED;
    }
...
edit_t* GetStateEdit(char which)
{
    ...
    {
        // If empty, generate
        if(history[s_curr + 1].undo.code == UNINITIALIZED)
        {
    ...
    // If empty, generate
    if(history[s_curr].redo.code == UNINITIALIZED){
