...
void OnChange(int from, int to){
    ...
    // Save an undo if the text size was increased
    if(newLines > 0){
        // Save new record with setlen with val = -newLines;
        ChainRecord(history, &history->next);
        history = history->next;
        InitEdit(history->undo, CUS_SETLENGTH, -newLines);
    }
...
void PerformUndos(int amount){
    ...
    while(history->prev != NULL && amount > 0){
        ...
        if(history->prev->time < history->time) amount--;
...
void PerformRedos(int amount){
    while(history->next != NULL && amount > 0){
        ...
        if(history->next->time > history->time) amount--;
        history = history->next;
    }
}
